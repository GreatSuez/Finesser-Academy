...
type StateWithDefaults = StateProps & { currentPage: number; childrenLength: number };
/**
 * Animates pages in cycle
 * (loop possible if children count > 1)
 */
class Carousel extends Component<CarouselProps, StateProps> {
  offset: number;
  nextPage: number;
  ...
  componentDidUpdate({ children }: CarouselProps) {
    if (!isEqual(this.props.children, children)) {
      const { currentPage } = this.state as StateWithDefaults;
      this._clearTimer && this._clearTimer();
  ...
  animateToPage = (page: number) => {
    const {
      ...
      childrenLength,
      size: { width },
    } = this.state as StateWithDefaults;
  ...
  _normalizePageNumber = (page: number) => {
    const { childrenLength } = this.state as StateWithDefaults;
  ...
  _renderPageInfo = (pageLength: number) => (
    <View
      style={[styles.pageInfoBottomContainer, this.props.pageInfoBottomContainerStyle]}
      pointerEvents="none"
    >
  ...
  _renderArrows = () => {
    let { currentPage } = this.state as StateWithDefaults;
    const { childrenLength } = this.state as StateWithDefaults;
    if (currentPage < 1) {
  ...
  render() {
    ...
    const { size, childrenLength } = this.state as StateWithDefaults;
      ...
      <View onLayout={this._onLayout} style={[this.props.style]}>
        <ScrollView
          ...
          contentContainerStyle={[
            ...
            {
              width:
                size.width * (childrenLength + (childrenLength > 1 && this.props.isLooped ? 2 : 0)),
              height: size.height,
            },
          ]}
        >
